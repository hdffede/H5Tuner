...
# The defualt value for stripe_count is 1. We are going to skip 1 and 4 because they
...
def eval_func(genome):
   ...
   print "Please evaluate this config for me(%d, %d, %d, %d, %d, %d): " % (this_strp_fac, this_strp_unt, this_cb_nds, this_align_thresh, this_align, this_siv_buf_size)
   #sys.stdout.flush()
   ...
   file_path = home_dir + '/.auto_tuner.conf'
   config_file = open(file_path, 'w');
   config_file.write(doc.toprettyxml(indent="  "))
   config_file.close()

   print 'Going to run the job'
   #sys.stdout.flush()
